...
WSDL.prototype.objectToDocumentXML = function(name, params, ns, xmlns, type,without_tns) {
  var args = {};
  args[name] = params;
  ...
  return this.objectToXML(args, null, ns, xmlns, true, null, parameterTypeObj,null,without_tns);
};
...
WSDL.prototype.objectToXML = function(obj, name, namespace, xmlns, first, xmlnsAttr, parameterTypeObject, ancestorXmlns,without_tns) {
  var self = this;
  var parts = [];

  var xmlnsAttrib = (first &&  (without_tns || xmlns))
    ? ((namespace && namespace !== 'xmlns' ? ' xmlns:' + namespace + '="' + xmlns + '"' : '') + ' xmlns="' + xmlns + '"')
    : '';
  ...
  if (Array.isArray(obj)) {
    for (var i = 0, item; item = obj[i]; i++) {
      ...
      parts.push(self.objectToXML(item, name, namespace, xmlns, false, null, null, ancXmlns, without_tns));
    ...
    for (name in obj) {
      ...
      var value = '';
      if (first) {
        value = self.objectToXML(child, name, namespace, xmlns, false, null, parameterTypeObject, ancXmlns, without_tns);
      } else {
        ...
        if (self.definitions.schemas) {
          ...
          if (schema) {
            ...
            if (childParameterTypeObject && childParameterTypeObject.$type && (childParameterTypeObject.$type.indexOf('xsd') === -1)) {
              ...
              value = self.objectToXML(child, name, childNamespace, childXmlns, false, childXmlnsAttrib, completeChildParameterTypeObject, ancXmlns, without_tns);
              ...
              value = self.objectToXML(child, name, obj.attributes.xsi_type.namespace, obj.attributes.xsi_type.xmlns, false, null, null, ancXmlns,without_tns);
            } else {
              value = self.objectToXML(child, name, namespace, xmlns, false, null, null, ancXmlns,without_tns);
            }
          } else {
            value = self.objectToXML(child, name, namespace, xmlns, false, null, null, ancXmlns, without_tns);
          }
